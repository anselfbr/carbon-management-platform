...
def _read_site_tbc_supplier_map(wb) -> dict[str, dict[str, str]]:
    """Read site-specific TBC supplier rows from Dropdown Values.

    Rule:
    - Supplier Name (optional) value should be like
      "常州廠(A9)-IPS_TBC - TBC".
    - The site key is the text before "_TBC - TBC", e.g. "常州廠(A9)-IPS".
    - Transportation Origin is read from the same row in Dropdown Values.
    """
    if "Dropdown Values" not in wb.sheetnames:
        return {}

    ws = wb["Dropdown Values"]
    supplier_header_keys = {
        _normalize_template_header("Supplier Name (optional)"),
        _normalize_template_header("Supplier Name"),
        _normalize_template_header("supplier_name"),
    }
    origin_header_keys = {
        _normalize_template_header("Transportation Origin"),
        _normalize_template_header("transportation_origin"),
        _normalize_template_header("Transport Origin"),
        _normalize_template_header("運輸起點"),
    }

    supplier_col = None
    origin_col = None
    for col in range(1, ws.max_column + 1):
        header_key = _normalize_template_header(ws.cell(1, col).value)
        if supplier_col is None and header_key in supplier_header_keys:
            supplier_col = col
        if origin_col is None and header_key in origin_header_keys:
            origin_col = col
    ...
    for row in range(2, min(ws.max_row, 20000) + 1):
        supplier_name = str(ws.cell(row, supplier_col).value or "").strip()
        if not supplier_name:
            continue

        upper_name = supplier_name.upper()
        if "TBC" not in upper_name:
            continue

        site_name = ""
        marker = "_TBC - TBC"
        upper_marker = marker.upper()
        idx = upper_name.find(upper_marker)
        if idx >= 0:
            site_name = supplier_name[:idx].strip()
        else:
            # Fallback for spacing variants, still require the site prefix before _TBC.
        ...
        origin = str(ws.cell(row, origin_col).value or "").strip() if origin_col else ""
        tbc_map[_normalize_col(site_name)] = {
            "supplier_name": supplier_name,
            "transportation_origin": origin,
        }

    return tbc_map


def _get_tbc_supplier_for_destination(
    tbc_supplier_map: dict[str, dict[str, str]],
    transportation_destination: Any,
) -> dict[str, str] | None:
    """Return the TBC supplier row for the same Transportation Destination."""
    destination = _normalize_col(transportation_destination)
    if not destination:
        return None
    if destination in tbc_supplier_map:
        return tbc_supplier_map[destination]

    destination_key = _normalize_template_header(destination)
    for site, item in tbc_supplier_map.items():
        if _normalize_template_header(site) == destination_key:
            return item
    return None
...
def _write_raw_material_bulk_from_exploded(
    ...
    site_tbc_supplier_map = _read_site_tbc_supplier_map(wb)
    ...
    for _, r in exploded.iterrows():
        ...
        transport_destination = r.get("transport_destination", "")
        tbc_supplier = _get_tbc_supplier_for_destination(site_tbc_supplier_map, transport_destination)
        supplier_name_value = tbc_supplier.get("supplier_name", "") if tbc_supplier else ""
        transport_origin_value = tbc_supplier.get("transportation_origin", "") if tbc_supplier else ""

        _write_template_value(activity_ws, row_idx, activity_cols["transport_origin"], transport_origin_value)
        _write_template_value(activity_ws, row_idx, activity_cols["transport_destination"], transport_destination)
        _write_template_value(activity_ws, row_idx, activity_cols["supplier_name"], supplier_name_value)
    ...
    return {
        ...
        "site_tbc_supplier_count": int(len(site_tbc_supplier_map)),
    }
