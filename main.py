...
def _find_module2_step1_output_source() -> tuple[Path | None, str]:
    """Return the Step1 Output source for Module 2.

    Priority:
    1) the Step1 Output uploaded in the current running session;
    2) a stable uploaded copy saved on disk;
    3) the latest Module 1 Step 1 output generated by this app.

    This lets users skip re-uploading Step1 Output on repeated Module 2 runs
    while avoiding accidental dependency on an old Module 1 latest file whenever
    an explicit Module 2 upload has already been provided.
    """
    global MODULE2_STEP1_OUTPUT_PATH

    if MODULE2_STEP1_OUTPUT_PATH and MODULE2_STEP1_OUTPUT_PATH.exists():
        return MODULE2_STEP1_OUTPUT_PATH, "session_uploaded"

    if LATEST_MODULE2_STEP1_OUTPUT_PATH.exists():
        MODULE2_STEP1_OUTPUT_PATH = LATEST_MODULE2_STEP1_OUTPUT_PATH
        return LATEST_MODULE2_STEP1_OUTPUT_PATH, "cached_upload"

    latest = _find_latest_module1_step1_output()
    if latest and latest.exists():
        return latest, "module1_latest_output"
...
def module2_step1_output_source():
    step1_path, source_mode = _find_module2_step1_output_source()
    if not step1_path:
        return {
            "ok": False,
            "source_mode": source_mode,
            "message": "尚未找到可供 Module 2 使用的 Step1 Output；請上傳 Module 1 Step1 Output，或先完成 Module 1 → Step 1。",
        }
    ...
    return {
        ...
        "download_url": f"/download/{step1_path.name}" if step1_path.parent == OUTPUT_DIR else "",
        "can_reuse_without_upload": source_mode in ["session_uploaded", "cached_upload"],
    }


def _find_latest_module2_raw_material_bulk() -> Path | None:
    """Return the stable Module 2 raw material bulk workbook for Module 3.

    Priority:
    1) In-memory path from the current Module 2 run.
    2) Stable latest workbook copied by Module 2.
    3) Most recent all-sites workbook.
    4) Most recent legacy single workbook.

    Split-by-site ZIP files are intentionally ignored because Module 3 needs one
    complete xlsx source, not one site workbook.
    """
    ...
    latest_path = OUTPUT_DIR / "raw_material_activity_data_bulk_latest.xlsx"
    if latest_path.exists():
        MODULE2_RAW_MATERIAL_BULK_PATH = latest_path
        return latest_path

    candidates: list[Path] = []
    for pattern in ["raw_material_activity_data_bulk_all_sites_*.xlsx", "raw_material_activity_data_bulk_*.xlsx"]:
        for path in OUTPUT_DIR.glob(pattern):
...
async def process_bom_expansion(request: Request):
    ...
    step1_file = form.get("step1_file") or form.get("step1_output_file")
    ...
    step1_source_mode = "not_found"

    template_path.write_bytes(await template_file.read())

    if is_upload_file_like(step1_file):
        step1_filename = str(getattr(step1_file, "filename", "") or "")
        if not step1_filename.lower().endswith((".xlsx", ".xlsm", ".xls")):
            return JSONResponse(
                {"ok": False, "message": "Module 1 Step1 Output 請上傳 Excel 檔案"},
                status_code=400,
            )
        step1_path = UPLOAD_DIR / f"module2_step1_output_{token}_{Path(step1_filename).name}"
        step1_path.write_bytes(await step1_file.read())
        try:
            shutil.copy2(step1_path, LATEST_MODULE2_STEP1_OUTPUT_PATH)
            MODULE2_STEP1_OUTPUT_PATH = LATEST_MODULE2_STEP1_OUTPUT_PATH
            step1_path = LATEST_MODULE2_STEP1_OUTPUT_PATH
        except Exception:
            traceback.print_exc()
            MODULE2_STEP1_OUTPUT_PATH = step1_path
        step1_source_mode = "uploaded"
    else:
        step1_path, step1_source_mode = _find_module2_step1_output_source()

    if step1_path is None or not step1_path.exists():
        return JSONResponse(
            {"ok": False, "message": "請先上傳 Module 1 Step1 Output，或先完成 Module 1 → Step 1 後再執行 Module 2。"},
            status_code=400,
        )
    ...
    try:
        ...
        # CMP V22.4: keep a stable all-sites Raw Material Bulk workbook for Module 3.
        # The user-facing Module 2 download remains the split-by-site ZIP, while
        # Module 3 always reads one complete xlsx source covering all Production Sites.
        MODULE2_RAW_MATERIAL_BULK_PATH = None
        latest_raw_bulk_path = OUTPUT_DIR / "raw_material_activity_data_bulk_latest.xlsx"
        all_sites_filename = str(summary.get("all_sites_output_filename") or "").strip()
        all_sites_path = OUTPUT_DIR / all_sites_filename if all_sites_filename else None
        if all_sites_path and all_sites_path.exists():
            try:
                shutil.copy2(all_sites_path, latest_raw_bulk_path)
                ...
                summary["module2_raw_material_bulk_source_filename"] = all_sites_path.name
                summary["module2_raw_material_bulk_source_scope"] = "all_sites"
            except Exception:
                traceback.print_exc()

        summary["module1_step1_source_filename"] = step1_path.name
        summary["module1_step1_source_mode"] = step1_source_mode
        summary["module1_step1_source_download_url"] = f"/download/{step1_path.name}" if step1_path.parent == OUTPUT_DIR else ""
        summary["module2_step1_cached_filename"] = LATEST_MODULE2_STEP1_OUTPUT_PATH.name if LATEST_MODULE2_STEP1_OUTPUT_PATH.exists() else ""
