...
async def process(request: Request):
    ...
    Fully 422-proof version:
    - No File(...) / Form(...) parameters in function signature
    - Read multipart form manually from request.form()
    - Accept files / file / labor_files / labor_file
    - Return controlled JSON errors instead of FastAPI validation 422
    """
    try:
    ...
    for key in ("labor_files", "labor_file"):
        values = form.getlist(key)
        for item in values:
            if isinstance(item, UploadFile) and item.filename:
    ...
    for upload in labor_uploads:
        ...
        saved = UPLOAD_DIR / f"labor_{uuid.uuid4().hex}_{filename}"
        saved.write_bytes(await upload.read())
