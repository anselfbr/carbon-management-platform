...
v1.5 更新：
- Input Sheet Products 的 A欄 Product Name 改為公式：
  ='Input Sheet Activity Data'!A{row}
- 不再將 A欄寫成純文字料號
- C欄 Product Description 仍由 Step1 Output 的 Material Description 帶入
- D欄 System Boundary = Cradle-to-Gate
- F欄 Declared Unit = PC

目的：
保留 bulk template 原本的公式連動邏輯，讓 Products 分頁的 Product Name 直接來自 Activity Data 分頁。
